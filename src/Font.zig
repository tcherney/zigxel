    ...
    pub fn draw_char(self: *Self, graphics: anytype, character: u8) !void {
        ...
        if (glyph_outline) |outline| {
            ...
            for (0..outline.curves.len) |i| {
                graphics.draw_bezier(.{ .r = 255, .g = 255, .b = 255 }, .{ .x = @as(i32, @intCast(outline.curves[i].p0.x)), .y = @as(i32, @intCast(outline.curves[i].p0.y)) }, .{ .x = @as(i32, @intCast(outline.curves[i].p1.x)), .y = @as(i32, @intCast(outline.curves[i].p1.y)) }, .{ .x = @as(i32, @intCast(outline.curves[i].p2.x)), .y = @as(i32, @intCast(outline.curves[i].p2.y)) }, tex.*);
            }
