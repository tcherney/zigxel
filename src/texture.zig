...
pub fn Texture(comptime T: ColorMode) type {
    return struct {
        ...
        pub fn rect(self: *Self, x: usize, y: usize, width: usize, height: usize, r: u8, g: u8, b: u8) Error!void {
            self.x = x;
            self.y = y;
            self.height = height;
            self.width = width;
        ...
        pub fn load_image(self: *Self, x: usize, y: usize, img: anytype) Error!void {
            self.x = x;
            self.y = y;
            ...
            for (0..self.pixel_buffer.len) |i| {
                self.pixel_buffer[i] = utils.rgb_256(img.data.items[i].r, img.data.items[i].g, img.data.items[i].b);
            }
        }
